...
export default function Chat() {
  ...
  const [officesMap, setOfficesMap] = useState({});  // ← NEW
  ...
  // ✅ NEW: Load offices names for shared rooms display
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      const batchSize = 10;
      const batches = [];
      for (let i = 0; i < roomIds.length; i += batchSize) batches.push(roomIds.slice(i, i + batchSize));
      ...
      batches.forEach((batch) => {
        const roomsQuery = query(collection(db, "rooms"), where("__name__", "in", batch));
        const unsubRoom = onSnapshot(roomsQuery, (roomSnap) => {
          ...
          completed++;
          if (completed === batches.length) { setRooms(allRooms); setLoading(false); }
        });
  ...
  useEffect(() => {
    ...
    const unsubA = onSnapshot(qA, (snapA) => { 
      ...
      completed++; 
      if (completed === 2) setSharedRooms(Array.from(new Map(rooms.map(item => [item.id, item])).values())); 
    });
  ...
  const handleTyping = useCallback(() => {
    ...
    const typingRef = doc(db, "typing", `${activeRoom.id}_${userData.uid}`);
    updateDoc(typingRef, {
    ...
    typingTimeoutRef.current = setTimeout(() => {
      deleteDoc(doc(db, "typing", `${activeRoom.id}_${userData.uid}`));
    }, 3000);
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', height: '100%', justifyContent: 'center', alignItems: 'center', background: COLORS.bg, color: COLORS.text }}>
        ⏳ جاري تحميل البيانات...
      </div>
    );
  }
  ...
  return (
    <div style={containerStyle}>
      <ChatSidebar 
        ...
        officesMap={officesMap}        // ← NEW
        userOfficeId={userData?.officeId}  // ← NEW
        activeRoomId={roomId}
        isOpen={sidebarOpen}
